...
#define kThumbnailLength  ((CGRectGetWidth([UIApplication sharedApplication].keyWindow.bounds) - 25.0)/4.0)
...
- (void)aWindowBecameVisible:(NSNotification *)noti
{
    UIWindow *theWindow = [noti object];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (!unFirst) {
        columns=floor(self.view.frame.size.width/(kThumbnailSize.width+kMinimumInteritemSpacing));
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.delegate=self;
//原来(self.tableView.frame.size.width-kThumbnailSize.width*tempAssets.count-kMinimumInteritemSpacing*(tempAssets.count-1))/2
    float assetViewX = (self.tableView.frame.size.width-kThumbnailSize.width*4-kMinimumInteritemSpacing*(4-1))/2;
    [cell bind:tempAssets selectionFilter:picker.selectionFilter minimumInteritemSpacing:kMinimumInteritemSpacing minimumLineSpacing:kMinimumLineSpacing columns:columns assetViewX:assetViewX];
    NSLog(@"indexpath = %ld x= %f",(long)indexPath.row,(self.tableView.frame.size.width-kThumbnailSize.width*tempAssets.count-kMinimumInteritemSpacing*(tempAssets.count-1))/2);
    return cell;
}
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    return kThumbnailSize.height+kMinimumLineSpacing;
}
...
@property (nonatomic, assign)BOOL isClickAlert;
...
- (void)viewDidLoad
{
    ...
    [self setupGroup];
    _isClickAlert = NO;
    self.resultTimer = [NSTimer scheduledTimerWithTimeInterval:1.0 target:self selector:@selector(refreshResult) userInfo:nil repeats:YES];
}
- (void)refreshResult
{
    if(_isClickAlert)
    ...
    if ((author == kCLAuthorizationStatusRestricted || author ==kCLAuthorizationStatusDenied)||(photoAuthorStatus ==PHAuthorizationStatusDenied || photoAuthorStatus ==PHAuthorizationStatusNotDetermined))
    {
        //无权限或有权限
        _isClickAlert = YES;
    }
...
- (void)showNotAllowed
{
    ...
    title.text              = NSLocalizedString(@"请允许此应用使用您的照片或视频。", nil);
