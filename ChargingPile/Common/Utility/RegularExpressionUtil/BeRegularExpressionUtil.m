...
@implementation BeRegularExpressionUtil
...
+ (BOOL)verifyIsChinese:(NSString *)string
{
    ...
    return [verifyCodePredicate evaluateWithObject:string];
}
//身份证号
    ...
    switch (length) {
        ...
        case 18:
            ...
            if(numberofMatch >0)
            {
                ...
                NSString *verifyM = [M lowercaseString];
                NSString *vefifyValue = [[value substringWithRange:NSMakeRange(17,1)] lowercaseString];
                if ([verifyM isEqualToString:vefifyValue]) {
                    return YES;// 检测ID的校验位
                }else {
                    return NO;
                }
...
+ (BOOL)validatCompanyName:(NSString *)companyName
{
    ...
    NSPredicate *emailTest = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", emailRegex];
    return [emailTest evaluateWithObject:companyName];
}
@end
